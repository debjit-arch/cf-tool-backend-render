...
router.get("/", authenticate, async (req, res) => {
  try {
    const users = await User.find().populate("department", "name"); // populate department
    const sanitized = users.map((u) => {
      const { password, ...rest } = u.toObject();
      return {
        ...rest,
        departmentId: u.department?._id, // add departmentId
        departmentName: u.department?.name, // add departmentName
      };
    });
    res.json(sanitized);
  } catch (err) {
