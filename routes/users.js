...
// Helper to get region-specific models
...
router.post(
  ...
  async (req, res) => {
    try {
      const { User, Department } = getModels(req.db);
      const { name, role, departmentId, email, password, organization } = req.body;
      ...
      if (role === "root" && req.user.role !== "super_admin")
        return res.status(403).json({ error: "Only super_admin can create root accounts" });

      if ((role === "risk_owner" || role === "risk_identifier") && req.user.role !== "root")
        return res.status(403).json({ error: "Only root can create risk owners or identifiers" });

      let userOrg;
      if (role === "super_admin") userOrg = undefined;
      else if (role === "root") {
        if (!organization) return res.status(400).json({ error: "Organization is required for root" });
      ...
      if (role === "risk_owner" || role === "risk_identifier") {
        if (!departmentId) return res.status(400).json({ error: "Department is required for this role" });
        const dep = await Department.findById(departmentId);
        if (!dep) return res.status(400).json({ error: "Invalid departmentId" });
        userDept = dep._id;
      }

      const exists = await User.findOne({ email: email.toLowerCase(), organization: userOrg });
      if (exists) return res.status(400).json({ error: "Email already exists in this organization" });

      const hashedPassword = bcrypt.hashSync(password, 10);

      const newUser = new User({ name, role, organization: userOrg, department: userDept, email: email.toLowerCase(), password: hashedPassword });
      await newUser.save();
...
router.post("/login", async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { id: user._id, role: user.role, organization: user.organization, departmentId: user.department?._id },
      JWT_SECRET,
      { expiresIn: "10h" }
    );
...
router.post("/change-password", authenticate, async (req, res) => {
  try {
    ...
    if (!oldPassword || !newPassword) return res.status(400).json({ error: "Old & new passwords are required" });

    const user = await User.findById(req.user.id);
    if (!user) return res.status(404).json({ error: "User not found" });
    if (!bcrypt.compareSync(oldPassword, user.password)) return res.status(401).json({ error: "Incorrect old password" });
...
router.post("/verify-otp", (req, res) => {
  try {
    ...
    if (store.otp !== otp) return res.status(400).json({ error: "Incorrect OTP" });
    if (Date.now() > store.expiresAt) return res.status(400).json({ error: "OTP expired" });
...
// UPDATE USER
// =============================
router.put("/:id", authenticate, authorizeRoles("super_admin", "root"), async (req, res) => {
  try {
    const { User, Department } = getModels(req.db);
    const user = await User.findById(req.params.id);
    if (!user) return res.status(404).json({ error: "User not found" });

    const { name, role, departmentId, email, password, organization, isAuditor } = req.body;

    if (req.user.role !== "root" && user.organization !== req.user.organization)
      return res.status(403).json({ error: "Cannot modify users from another organization" });

    if (role && !ALLOWED_ROLES.includes(role)) return res.status(400).json({ error: "Invalid role" });
    if (role === "super_admin" && req.user.role !== "root") return res.status(403).json({ error: "Only root can assign super_admin role" });
    ...
    if (organization && req.user.role === "root") user.organization = organization;
...
// DELETE USER
// =============================
router.delete("/:id", authenticate, authorizeRoles("super_admin", "root"), async (req, res) => {
  try {
    const { User } = getModels(req.db);
    ...
    if (user.role === "super_admin" && req.user.role !== "root")
      return res.status(403).json({ error: "Only root can delete a super_admin" });
